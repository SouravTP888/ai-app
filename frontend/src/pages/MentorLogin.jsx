...
import { Mail, Lock, LogIn, AlertCircle, Award } from 'lucide-react';

const MentorLogin = () => {
  const { mentorLogin, logout, user, error, setError } = useContext(AuthContext);
  ...
  const handleSubmit = async (e) => {
    ...
    setLoading(true);
    const success = await mentorLogin(email, password);
    setLoading(false);
  };

  const handleFillDemo = () => {
    setEmail('mentor@eduflick.ai');
    setPassword('mentorpassword');
  };
    ...
    <div className="min-h-screen bg-slate-900 grid-bg flex items-center justify-center p-6 relative">
      ...
      <div className="relative z-10 w-full max-w-md glass-panel p-8 rounded-3xl border border-slate-800 shadow-2xl flex flex-col">
        ...
        <div className="flex flex-col items-center mb-8">
          ...
          <h2 className="text-2xl font-black text-white tracking-tight">Mentor Portal</h2>
          <p className="text-slate-400 text-xs mt-1">Sign in with your mentor credentials</p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            ...
            <div className="relative">
              ...
              <input
                ...
                placeholder="mentor@eduflick.ai"
          ...
          <button
            type="submit"
            disabled={loading}
            className="w-full bg-brand-600 hover:bg-brand-500 text-white font-bold py-3 px-4 rounded-xl flex items-center justify-center gap-2 shadow-lg shadow-brand-500/10 disabled:opacity-50 transition-all duration-200 text-xs cursor-pointer"
          >
        ...
        <div className="mt-6 pt-5 border-t border-slate-800/80">
          <p className="text-[10px] uppercase font-bold text-slate-500 tracking-wider text-center mb-3">
            Quick Fill Mentor Demo
          </p>
          <button
            onClick={handleFillDemo}
            className="w-full py-2 px-3 rounded-lg bg-slate-800/50 hover:bg-slate-800 border border-slate-800 text-[10px] font-bold text-brand-300 transition-colors cursor-pointer"
          >
            Demo Mentor (mentor@eduflick.ai / mentorpassword)
          </button>
        </div>
